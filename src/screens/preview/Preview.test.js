...
jest.mock('../../../node_modules/gif.js/dist/gif');
jest.mock('../../../node_modules/upng-js/UPNG');
...
describe('Preview.prototype.constructor', () => {
  const frames = new Frames();
  const preview = new Preview(frames);

  it('should save frames', () => {
    expect(preview.frames).toEqual(frames);
  });

  it('should save isAnimation in false', () => {
    expect(preview.isAnimation).toBe(false);
  });

  it('should save FPS in 1', () => {
    expect(preview.FPS).toBe(1);
  });

  it('should save animationInterval in 1000/FPS', () => {
    expect(preview.animationInterval).toBe(1000 / preview.FPS);
  });
});

describe('Preview.prototype.animateFrames', () => {
  const frames = new Frames();
  frames.addEvents();
  ...
  preview.isAnimation = false;
  preview.animateFrames();
  expect(setTimeout).toHaveBeenCalledTimes(0);

  // this.isAnimation === true, frames.length === 1
  preview.isAnimation = true;
  preview.animateFrames();
  expect(setTimeout).toHaveBeenCalledTimes(0);

  // this.isAnimation === true, frames.length === 2
  frames.addFrame();
  preview.isAnimation = true;
  preview.animateFrames();
...
describe('exportToGif', () => {
  ...
  it('should call GIF constructor', () => {
    frames.addFrame();
    frames.addFrame();
    layers.addLayer();
    frames.addFrame();
